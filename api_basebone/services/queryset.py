...
def basebone_get_model_role_config(model):
    """获取角色配置"""
    role_config = getattr(
...
def get_user_role_filters(user, role_config):
    """获取此用户权限对应的过滤条件"""
...
def translate_expand_fields(model, expand_fields):
    """转换展开字段"""
...
def _guard(model, action, user, queryset):
    # 如果不是超级用户，则进行对应的数据筛选
...
def get_queryset_by_filter_user(user, model, role_config, queryset):
    """通过用户过滤对应的数据集
...
def get_queryset_by_order_by(fields, queryset):
    """结果集支持排序"""
...
def should_distinct_queryset(model, action, fields):
    """检测是否需要
...
def get_queryset_by_filter_conditions(user, model, action, filters, role_config, queryset, view=None):
    ...
    role_filters = get_user_role_filters(user, role_config)
...
def get_queryset_by_with_tree(tree_data, queryset):
    """如果是树形结构，则需要做对应的过滤"""
    if tree_data:
...
def queryset(request, model, action='list', filters=[], fields=[], expand_fields=[], order=[], tree_data=None, skip_distinct=False, view=None):
    ...
    if expand_fields:
        ...
        queryset = queryset_utils.queryset_prefetch(queryset, expand_dict, context, display_fields=fields)

    # 3. 添加计算字段的annotate
    if action not in ['get_chart', 'group_statistics']:
    ...
    role_config = basebone_get_model_role_config(model)
    ...
    # 11. 权限中配置是否去重
    if skip_distinct:  # 统计和图表的场景不需要distinct.
        return queryset
